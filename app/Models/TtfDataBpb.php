...
class TtfDataBpb extends Model
{
    ...
    public function validateBPBByBPBNumber($bpb_number){
        ...
        $data = TtfDataBpb::join('ttf_lines', 'ttf_lines.TTF_BPB_ID', '=', 'ttf_data_bpb.BPB_ISD')
              ->join('ttf_headers', 'ttf_headers.TTF_ID', '=', 'ttf_lines.TTF_ID')
              ->whereNotIn('ttf_headers.TTF_STATUS',['R','C'])
              ->where('ttf_data_bpb.USED_sFLAG','Y')
              ->where('ttf_data_bpb.BPB_NUMBER',$bpb_number)
              ->select('ttf_data_bpb.BPB_ID')->get();
        return $data;
    }
}
